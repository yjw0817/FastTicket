...
<%
  /**
  * @Class Name : userRoleRegister.jsp
  * @Description : 사용자-권한 부여 화면
  */
%>
...
<head>
    ...
    <script type="text/javaScript" language="javascript" defer="defer">
        ...
        function fn_egov_selectList() {
            document.detailForm.action = "<c:url value='/userRoleList.do'/>";
        ...
        function fn_egov_save() {
            ...
            frm.action = "<c:url value='/addUserRole.do'/>";
            frm.submit();
        }
...
<form:form modelAttribute="userRoleVO" id="detailForm" name="detailForm" method="post">
    <div id="content_pop">
        <!-- 타이틀 -->
        <div id="title">
            <ul>
                <li><img src="<c:url value='/images/ftk/title_dot.gif'/>" alt=""/>권한 부여</li>
            </ul>
        </div>
        <!-- // 타이틀 -->
        <div id="table" class="table-responsive">
            <table class="table" style="border-top:2px solid #C2D0DB; border-collapse:collapse;">
                ...
                <tr>
                    <td class="tbtd_caption"><label for="memberId">회원</label></td>
                    <td class="tbtd_content">
                        <form:select path="memberId" cssClass="form-select form-select-sm" style="width:auto;">
                            <form:option value="" label="-- 회원 선택 --" />
                            <c:forEach var="member" items="${memberList}">
                                <form:option value="${member.memberId}" label="${member.memberId} - ${member.memberNm}" />
                            </c:forEach>
                        </form:select>
                    </td>
                </tr>
