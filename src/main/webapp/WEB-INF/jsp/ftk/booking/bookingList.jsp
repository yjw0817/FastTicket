...
<head>
    ...
    <script type="text/javaScript" language="javascript" defer="defer">
        ...
        function fn_egov_addView() {
            location.href = "<c:url value='/addBooking.do'/>";
        }
                ...
                <table class="table table-hover table-striped"
                    ...
                    <c:forEach var="result" items="${resultList}" varStatus="status">
                        <tr>
                            ...
                            <td align="center" class="listtd">
                                <a href="javascript:fn_egov_select('<c:out value="${result.bookingId}"/>')">
                                    <c:out value="${result.bookingId}"/>
                                </a>
                            </td>
                            <td align="left" class="listtd">
                                <c:out value="${result.programNm}"/>&nbsp;
                            </td>
                            <td align="center" class="listtd">
                                <c:out value="${result.eventDate}"/>
                                <c:if test="${not empty result.eventTime}">
                                    &nbsp;<c:out value="${result.eventTime}"/>
                                </c:if>
                            </td>
                            <td align="center" class="listtd">
                                <c:out value="${result.bookerNm}"/>&nbsp;
                            </td>
                            <td align="center" class="listtd">
                                <c:out value="${result.bookerTel}"/>&nbsp;
                            </td>
                            <td align="center" class="listtd">
                                <c:out value="${result.totalQty}"/>
                            </td>
                            <td align="right" class="listtd">
                                <c:out value="${result.totalAmt}"/>&nbsp;
                            </td>
                            <td align="center" class="listtd">
                                <c:choose>
                                    <c:when test="${result.bookingStatus == 'RESERVED'}"><span class="badge-info">예매완료</span></c:when>
                                    <c:when test="${result.bookingStatus == 'CANCELED'}"><span class="badge-cancel">취소</span></c:when>
                                    <c:when test="${result.bookingStatus == 'COMPLETED'}"><span class="badge-active">완료</span></c:when>
                                    <c:otherwise><span class="badge-inactive"><c:out value="${result.bookingStatus}"/></span></c:otherwise>
                                </c:choose>
                            </td>
                            <td align="center" class="listtd">
                                <c:choose>
                                    <c:when test="${result.paymentStatus == 'PENDING'}"><span class="badge-pending">미결제</span></c:when>
                                    <c:when test="${result.paymentStatus == 'PAID'}"><span class="badge-active">결제완료</span></c:when>
                                    <c:when test="${result.paymentStatus == 'REFUNDED'}"><span class="badge-inactive">환불</span></c:when>
                                    <c:otherwise><span class="badge-inactive"><c:out value="${result.paymentStatus}"/></span></c:otherwise>
                                </c:choose>
                            </td>
                        </tr>
                    </c:forEach>
